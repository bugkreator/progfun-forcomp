...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    // treat w as sequence, group by its chars by their lower case values. get map of chars & strings of all those chars
    // convert to list and map (c,s) pair to (c, len(s)). then sort result by first element of each pair
    w.groupBy(_.toLower).toList.map( (p:(Char,String)) => (p._1, p._2.length)).sortBy(_._1)
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences match {
      case List() => List( List() )
      case (c: Char, a: Int)::tail => {
        val tailCombinations : List[Occurrences] = combinations(tail)
        val headCombinations : List[Occurrences] = List() :: Range(1,a+1).toList.map( (i:Int) => List((c,i)))
        (for (tc <- tailCombinations; hc <- headCombinations) yield  hc++tc).toList // hc++tc to maintain order of lists
      }
    }
  }
